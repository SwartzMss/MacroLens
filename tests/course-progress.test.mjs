...
test('damaged, future and unpublished progress cannot mark courses complete', () => {
  ...
  assert.deepEqual(parseCourseProgress(JSON.stringify({ version: 1, completed: ['connected-economy', 'connected-economy', 'policy', 5, '__proto__'], lastVisited: 'policy' })), { version: 1, completed: ['connected-economy'], lastVisited: null });
});
...
test('completion is explicit and reversible, including when persistence is unavailable', () => {
  ...
  assert.equal(toggleCourseCompletion(initial, 'policy'), initial);
...
test('resume handles fresh, out-of-order and cleared progress without entering unpublished chapters', () => {
  ...
  assert.equal(courseResumeTarget(parseCourseProgress('{"version":1,"completed":[],"lastVisited":"policy"}')).id, 'connected-economy');
});
