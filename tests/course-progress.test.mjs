...
import { parseCourseProgress, courseStorageKey, toggleCourseCompletion, saveCourseProgress } from '../src/data/courseProgress.ts';
...
test('damaged, future and unpublished progress cannot mark courses complete', () => {
  ...
  assert.deepEqual(parseCourseProgress(JSON.stringify({ version: 1, completed: ['connected-economy', 'connected-economy', 'money', 5, '__proto__'], lastVisited: 'money' })), { version: 1, completed: ['connected-economy'], lastVisited: null });
});
...
test('completion is explicit and reversible, including when persistence is unavailable', () => {
  ...
  assert.equal(toggleCourseCompletion(initial, 'money'), initial);
