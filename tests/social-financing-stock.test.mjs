...
import { nextMonth } from '../scripts/ingest/validate/dataset.ts';
import { validatePBOCFinancialDataset } from '../scripts/ingest/validate/pboc-credit-social-financing.ts';

test('social financing charts all stock amounts on the left and published growth on the right', () => {
  ...
  assert.ok(data.balance.data.length >= 31);
  assert.deepEqual(data.balance.data[0], { date: '2024-01', value: 384.32 });
  assert.deepEqual(data.balance.data.find(({ date }) => date === '2026-07'), { date: '2026-07', value: 463.27 });
  ...
  assert.deepEqual(option.series[1].data, data.balance.data.map(({ value }) => value));
...
test('social financing stock validation rejects invalid units, negative values and unmatched dates', () => {
  ...
  for (const mutate of [
    ...
    data => { data.balance.data.at(-1).date = nextMonth(data.data.at(-1).date); },
  ]) {
