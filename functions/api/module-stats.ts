import { combineModuleStats, moduleStatsQueries, parseModuleStats } from '../module-stats.ts';
...
export async function onRequest({ request, env }: Context): Promise<Response> {
  ...
  try {
    ...
    const queries = moduleStatsQueries();
    ...
    const total = parseModuleStats(totalPayload, 'total');
    const today = parseModuleStats(todayPayload, 'today');
    if (!total || !today) return unavailable();

    return Response.json(
      { available: true, modules: combineModuleStats(total, today) },
