...
export function validateIndicatorDataset(dataset: IndicatorDataset): void {
  ...
  for (const source of dataset.sources) {
    const coverage = source.coverage.match(COVERAGE_PATTERN);
    if (!source.title || !/^https:\/\/www\.stats\.gov\.cn\//.test(source.url) || !coverage || coverage[1] > coverage[2]) {
  ...
  for (const observation of dataset.data) {
    ...
    if (previousDate && nextMonth(previousDate) !== observation.date) fail(`PMI observations must be continuous: ${previousDate} -> ${observation.date}`);
  ...
  if (!coverageCoversDates(dataset.sources, dataset.data.map((observation) => observation.date))) {
    fail('PMI source coverage union does not fully cover the dataset');
  }
